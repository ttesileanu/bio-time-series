...
# %% [markdown]
# # Look at hyperoptimization results for BioWTA on various combinations of vowels
...
start_seed = 800
n_seeds = 40
batch_path = os.path.join("..", "..", "batch", "biowta_vowels_many")

vowels = "aeiou"
configurations = [
    vowels[i] + vowels[j] for i in range(len(vowels)) for j in range(i + 1, len(vowels))
]

results = {}
for vowel_pair in tqdm(configurations):
    crt_dict = {"fct": [], "fct_quantile": [], "fct_good": []}
    for crt_seed in range(start_seed, start_seed + n_seeds):
        # load the data
        fname = os.path.join(batch_path, f"test_{vowel_pair}_{crt_seed}.hdf5")
...
for key in results:
    ...
    fig, _ = make_summary_figure(
        crt_results,
        log_scale=["rate", "temperature", "exp_streak", "timescale"],
        fct_key="fct_good",
        vmin=0.0,
        vmax=1.0,
        top_fraction=0.05,
    )
    fig.suptitle(f"vowels: {key}")

# %%
summary = pd.DataFrame()
summary["mod_key"] = configurations

for fct_type in ["fct", "fct_quantile", "fct_good"]:
    summary[f"best_{fct_type}"] = [
        np.max(results[cfg][fct_type]) for cfg in configurations
    ]

keys = results["ao"].keys()
chosen_fct = "fct"
for key in keys:
