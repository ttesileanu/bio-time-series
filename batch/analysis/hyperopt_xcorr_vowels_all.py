...
# %% [markdown]
# # Look at hyperoptimization results for autocorrelation algorithm on mixed vowel data (all vowel pairs)
...
start_seed = 900
n_seeds = 40
batch_path = os.path.join("..", "..", "batch", "xcorr_vowels_all")

results = {"fct": [], "fct_quantile": [], "fct_good": [], "fct_highest": []}
all_data = {"params": [], "fct_scalar": []}
for crt_seed in tqdm(range(start_seed, start_seed + n_seeds)):
    ...
    all_data["params"].extend(crt_res["params"])
    all_data["fct_scalar"].extend(crt_res["fct_scalar"])
    ...
    for key in results:
        if key not in ["fct", "fct_quantile", "fct_good", "fct_highest"]:
    ...
    # highest score
    for crt_eval in crt_res["fct_full"]:
        crt_highest = np.max(crt_eval[1].trial_scores)
        results["fct_highest"].append(crt_highest)

# ensure everything is a Numpy array
for key in results:
    results[key] = np.asarray(results[key])
    
all_data["fct_scalar"] = np.asarray(all_data["fct_scalar"])
...
fig, _ = make_summary_figure(
    results,
    log_scale=["rate", "exp_streak", "feature_step"],
    fct_key="fct_good",
    vmin=0.0,
    vmax=1.0,
    top_fraction=0.01,
...
fig.savefig(os.path.join(fig_path, "xcorr_hyperopt_vowel.pdf"))
...
for fct_type in ["fct", "fct_quantile", "fct_good", "fct_highest"]:
...
with FigureManager() as (_, ax):
    ax.hist(
        [
            __
            for _, __ in zip(all_data["params"], all_data["fct_scalar"])
            if _["feature_step"] == 1
        ],
        25,
    )
    ...
    ax.set_ylabel("Count")
    ax.set_title("Segmentation performance for feature_step = 1")
