...
def fit_infer_ar(regressor: Any, y: Sequence, bias: bool = False, **kwargs) -> Any:
    ...
    calls `regressor.fit_infer` to fit a regression model and infer the underlying
    ...
    returned by `regressor.fit_infer`.

    Parameters
    ----------
    regressor
        Regression object to use for fitting. This should have a method `fit_infer` like
    ...
    bias
        ...
        non-zero mean
    All other keyword arguments are passed directly to `regressor.fit_infer`.

    Returns the output from `regressor.fit_infer(Xar, yar)` called with `yar = y[p:]`
    ...
    return regressor.fit_infer(X[p - 1 : -1], y[p:], **kwargs)
